...
struct _BlowFish_Data {
	void *data;
	unsigned long long size;
};

struct _BlowFish_Key {
	void *key;
	short size;
};

BlowFish * blowfish_new(BlowFish_Key *key){
	BlowFish *bfish = (BlowFish*)malloc(sizeof(BlowFish));
	int i,j;
	for(i = 0;i < 18;i++){
		bfish->p[i] = parray[i];
	}
...
BlowFish_Data * blowfish_encrypt(BlowFish_Key *key, BlowFish_Data *ctext){
	BlowFish *bfish = blowfish_new(key);
	BlowFish_Data *data = NULL;
	return data;	
}
